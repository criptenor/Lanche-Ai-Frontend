...
class _AnaliticsWidgetState extends State<AnaliticsWidget>
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      _model.apiResultvj9 = await GetMetricasCall.call(
        idLoja: FFAppState().IDAPP,
      );
      if ((_model.apiResultvj9?.succeeded ?? true)) {
        setState(() {
          FFAppState().analitics = GetMetricasStruct(
            pixPercentual: GetMetricasCall.pix(
              (_model.apiResultvj9?.jsonBody ?? ''),
            ),
            cartaoPercentual: GetMetricasCall.cartao(
              (_model.apiResultvj9?.jsonBody ?? ''),
            ),
            dinheiroPercentual: GetMetricasCall.dinheiro(
              (_model.apiResultvj9?.jsonBody ?? ''),
            ),
            vendasUltimas24h: GetMetricasCall.vendas(
              (_model.apiResultvj9?.jsonBody ?? ''),
            ),
            horarioDePico: GetMetricasCall.horarioDePico(
              (_model.apiResultvj9?.jsonBody ?? ''),
            ),
            totalPix: GetMetricasCall.totalPix(
              (_model.apiResultvj9?.jsonBody ?? ''),
            ),
            totalCartao: GetMetricasCall.totalCard(
              (_model.apiResultvj9?.jsonBody ?? ''),
            ),
            totalDinhreiro: GetMetricasCall.totalDinheiro(
              (_model.apiResultvj9?.jsonBody ?? ''),
            ),
            tempomediototal: GetMetricasCall.tempoMEntrega(
              (_model.apiResultvj9?.jsonBody ?? ''),
            ),
            tempomediopedido: GetMetricasCall.velocidadedaproducaohoje(
              (_model.apiResultvj9?.jsonBody ?? ''),
            )?.toString(),
          );
        });
                                  ...
                                  child: Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        FutureBuilder<List<CompraRow>>(
                                          future: CompraTable().queryRows(
                                            queryFn: (q) => q
                                                .eq(
                                                  'id_loja',
                                                  FFAppState().IDAPP,
                                          ...
                                          builder: (context, snapshot) {
                                            ...
                                            return ListView.builder(
                                              ...
                                              scrollDirection: Axis.vertical,
                                              itemCount:
                                                  listViewCompraRowList.length,
                                              itemBuilder:
                                                  (context, listViewIndex) {
                                                final listViewCompraRow =
                                                    listViewCompraRowList[
                                                        listViewIndex];
                                                return Padding(
                                                  padding: EdgeInsetsDirectional
